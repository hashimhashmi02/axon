...
import { CheckCheckIcon, XCircleIcon, Loader2Icon, ClockIcon } from "lucide-react";
...
export const ExecutionView = ({
    ...
    const [showsStackTrace, setShowTrace] = useState(false);

    const duration = execution.completedAt
        ? Math.round(
            (new Date(execution.completedAt).getTime() - new Date(execution.statedAt).getTime()) / 1000
        )
        : null;
            ...
            <CardHeader>
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <CardDescription>
                            Execution for {execution.workflow?.name ?? "Unknown workflow"}
                        </CardDescription>
                    </div>
                </div>
            </CardHeader>
            <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-sm">{formatDistanceToNow(execution.statedAt, { addSuffix: true })}</p>
                    </div>
                        ...
                        {execution.errorStack && (
                            ...
                            >
                                <CollapsibleTrigger asChild>
                                    ...
                                    >
                                        {showsStackTrace ? "Hide stack trace" : "Show stack trace"}
                                    </Button>
                                </CollapsibleTrigger>
                                <CollapsibleContent>
